...
async def run_campaign_execution(campaign_id: str, campaign_data: dict = None, recipients: list = None):
    ...
    try:
        ...
        if not recipients:
            ...
            target_audience_clean = target_audience.strip().lower() if isinstance(target_audience, str) else "all"
            
            if target_audience_clean in ["all", "all candidates", "all_candidates", "everyone", "any"]:
...
async def execute_campaign_endpoint(request: ExecutionRequest, background_tasks: BackgroundTasks, usage_allowed: bool = Depends(verify_usage_limit), current_user: User = Depends(get_current_user)):
    try:
        ...
        # Publish to Redis Queue (PubSub) or Fallback
        try:
            if hasattr(event_queue, "redis") and event_queue.redis:
                queued = await event_queue.publish("campaign_events", {"type": "execute_campaign", "campaign_id": request.campaign_id})
                if not queued:
                    raise Exception("Redis PubSub publish returned False")
                logging.info(f"Published execute_campaign event to Redis PubSub for campaign {request.campaign_id}")
            else:
                raise Exception("Redis not configured for event_queue")
        except Exception as e:
