    ...
    Papa.parse(file, {
      ...
      complete: (results) => {
        ...
        results.data.forEach((row: any) => {
          ...
          if (candidate.name && (candidate.phone || candidate.email)) {
            // Basic validation to ensure phone is somewhat valid (clean it) if present
            if (candidate.phone) {
        ...
        if (parsedData.length === 0) {
          toast({ title: "No valid candidates found", description: "Check CSV headers (name, phone or email)", variant: "destructive" });
          return;
        }
